"""Get cafeteria menu from KAIST website."""
...
class InvalidCafeteriaError(ValueError):
    ...
    def __init__(self, cafeteria: str):
        super().__init__(
            f"Invalid cafeteria code: {cafeteria}, "
            f"must be one of {get_args(Cafeteria)}"
        )
...
def _make_url(cafeteria: Cafeteria, dt: datetime.datetime) -> str:
    return (
...
async def get_menu(cafeteria: Cafeteria, dt: datetime.datetime | None = None) -> Menu:
    """
    ...
    dt : datetime.datetime, optional
        Datetime object representing the date to retrieve the menu for. If None, the
...
async def get_menus(
    cafeteria_list: list[Cafeteria], dt: datetime.datetime | None = None
) -> list[Menu]:
    """
